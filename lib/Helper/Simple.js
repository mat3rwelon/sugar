 ...
 */
exports.getImageLink = async (buffer, apiKey) => {
	const options = {
		apiKey: global?.apiKey?.imgbb || apiKey,
...
const getImageLinkV2 = async buffer => {
...
exports.getImageLinkV2 = getImageLinkV2
exports.telegraph = getImageLinkV2
...
const toNumber = (jid) => {
	if (jid.includes('@')) {
		...
		let number = jid.toString()
		number = number.replace(/(\++)?(\s+)?(\-+)?(\@+)?(\.+)?([a-zA-Z]+)?/g, '')
		number = number.startsWith('08') ? '628' + number.slice(2) : number
		return number
	}
}
const logg = (...o) => console.log(...o)
